...
function scene:create(e)
	...
	cookies = display.newImageRect("images/cookies.png", 220, 170)
	cookies.alpha = 0
	
	--Create some title text
	local font = Prefs.header.font
	local size = Prefs.header.size
	titleText = display.newText("Search Cookies", 0, 0, font, size)
	titleText.alpha = 0
	...
	transition.from(cookies, {
		time = 1000,
		yScale = 0.1,
		xScale = 0.1,
		transition = easing.outBounce,
		onComplete = function()
			print("cookies complete")
		end
	})
